...
//					jPanel_ceter.setBorder(titledBorder_dssach);
				...
				//Tao bảng
					JPanel jPanel_ceter_table = new JPanel();
						...
						Dimension size = new Dimension(980, 500);
...
//			this.add(jPanel_ceter_dssach);
	...
	public boolean kiemTraTuaSach_TacGia(String ts_tg) {
		for(int i = 0; i<ts_tg.length(); i++) {
			char c = ts_tg.charAt(i);
	...
	private void themSach() throws Exception{
		...
		else if(s.getMaSach().charAt(0)==s.getTuaSach().charAt(0) && kyTuBaKyTuCuoiMaSo(s.getMaSach()) ){
	...
	public void loadData(){
		...
		if(danhSachCacSach==null) {
			...
			for (Sach s : danhSachCacSach.getDsSach()) {
				String hang[] = {s.getMaSach(), s.getTuaSach(), s.getTacGia(), s.getNamXuatBan()+"", s.getNhaXuatBan(), s.getSoTrang()+""
