...
public class View extends JFrame implements ActionListener, MouseListener{
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(src.equals("Thêm"))
		{
				try {
					this.themnv();
					databasee.savefile("Nhanvien.dat", danhSachNhanVien);
				} catch (Exception e1) {
		...
		}else if(src.equals("Xoá trắng"))
		{
			...
			int awt = JOptionPane.showConfirmDialog(this, "Lưu file thành công", "Thông báo", JOptionPane.WARNING_MESSAGE);
			{
				this.luu();			
			}
	...
	public void capnhat() {
		...
		NhanVien nv = new NhanVien(maNv, ho, ten, tuoi, phai, tienluong);
			...
			JOptionPane.showConfirmDialog(this, "Ch�?n dòng cần cập nhật nha bạn!!!","Thông báo", JOptionPane.OK_OPTION);
		}
	}
	private void tim() {
	...
	private void xoanv(){
		...
		if(hang!=-1)
		{
			...
			if(anw ==JOptionPane.YES_OPTION )
			{
				...
				JOptionPane.showConfirmDialog(this, "Ch�?n dòng cần xoá");
			}
	}
	...
	private void themnv() throws Exception{
		...
		double tienluong= Double
				.parseDouble(jTextField_tienluong.getText());		
		NhanVien nv = new NhanVien(maNv, ho, ten, tuoi, phai, tienluong);
	...
	public void loadData() throws Exception {
		...
		danhSachNhanVien= (DanhSachNhanVien) databasee.readfile("Nhanvien.dat");
		if(danhSachNhanVien==null) {
			danhSachNhanVien = new DanhSachNhanVien();
		}else {
			for (NhanVien nv : danhSachNhanVien.getDsNhanVien()) {
				String hang[] = {nv.getMaNV(), nv.getHo(), nv.getTen(), nv.getTuoi()+"", nv.getPhai(), nv.getTienLuong()+""};
